...
function setUserResponse(message) {
  const user_response = `<img class="userAvatar" src='/static/chatbot_static/img/userAvatar.jpg'><p class="userMsg">${message} </p><div class="clearfix"></div>`;
...
function getBotResponse(text) {
  botResponse = `<img class="botAvatar" src="/static/chatbot_static/img/sara_avatar.png"/><span class="botMsg">${text}</span><div class="clearfix"></div>`;
  return botResponse;
}
...
function setBotResponse(response) {
  ...
  setTimeout(() => {
    ...
    if (response.length < 1) {
      ...
      const BotResponse = `<img class="botAvatar" src="/static/chatbot_static/img/sara_avatar.png"/><p class="botMsg">${fallbackMsg}</p><div class="clearfix"></div>`;
        ...
        if (Object.hasOwnProperty.call(response[i], "text")) {
          if (response[i].text != null) {
            ...
            if (
              ...
              if (!botResponse) {
                botResponse = `<img class="botAvatar" src="/static/chatbot_static/img/sara_avatar.png"/><p class="botMsg">${response[i].text}</p><div class="clearfix"></div>`;
              }
            }
...
window.addEventListener('message', function(event) {
  if (event.data.type === 'initData') {
    ...
    initializeChatbot(userId, scenarioId);

    const customEvent = new CustomEvent('userDataReceived', {
      detail: { userId: userId, scenarioId: scenarioId }
    });
    window.dispatchEvent(customEvent);
  } else if (event.data.type === 'pendulumData') {
  ...
  $.ajax({
    ...
    success(botResponse, status) {
      ...
      botResponse.forEach((message) => {
        // Handle custom activity ID if available
        if (message.custom && message.custom.activity_id !== undefined) {
          console.log("Activity ID: ", message.custom.activity_id);
        ...
        // Handle buttons, assuming each button might have an activity_id
        if (message.buttons) {
          message.buttons.forEach((button) => {
            if (button.activity_id !== undefined) {
              console.log("Button:", button.title, "Activity ID:", button.activity_id);
              const buttonActivityEvent = new CustomEvent('activityIdReceived', { detail: { activityId: button.activity_id } });
              window.dispatchEvent(buttonActivityEvent);
            }
          });
        }
      });
